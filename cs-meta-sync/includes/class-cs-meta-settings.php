...
class CS_Meta_Settings {
    ...
    public function register_settings() {
        ...
        add_settings_section(
            ...
            function () {
                echo '<p>' . esc_html__( 'Choose how often WooCommerce products are fully re-synced to your Meta catalog.', 'cs-meta-sync' ) . '</p>';
            },
            'cs-meta-sync'
        );
    ...
    public function sanitize_settings( $input ) {
        ...
        if ( ( $old_options['sync_interval'] ?? '' ) !== $sanitized['sync_interval'] ) {
            $timestamp = wp_next_scheduled( 'cs_meta_catalog_sync' );
            if ( $timestamp ) {
                wp_unschedule_event( $timestamp, 'cs_meta_catalog_sync' );
            }
            wp_schedule_event( time(), $sanitized['sync_interval'], 'cs_meta_catalog_sync' );
        }
